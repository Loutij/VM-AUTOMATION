#!/usr/bin/env python3
"""
Script pour tester la connexion PowerShell Direct avec DEMO_VM (vérifier le correctif).
"""
...
async def test_demo_vm():
    """Teste la connexion PowerShell Direct avec DEMO_VM."""
    async with db_session() as session:
        from sqlalchemy import select
        
        # Trouver le déploiement DEMO_VM
            ...
            for username, password in credentials_list:
                try:
                    ...
                    if result.success:
                        ...
                        # Test du fichier ready.flag
                        print("\n🔍 Vérification du fichier ready.flag...")
                        flag_script = 'if (Test-Path "C:\\VM-Automation\\ready.flag") { "FLAG_FOUND" } else { "FLAG_NOT_FOUND" }'
                        flag_result = await client.execute_in_vm(
                            vm.name,
                            flag_script,
                            (username, password),
                            timeout=30
                        ...
                        if flag_result.success:
                            output = str(flag_result.output).strip()
                            if "FLAG_FOUND" in output:
                                print("   ✅ ready.flag trouvé!")
                            else:
                                print("   ⚠️  ready.flag non trouvé")
...
if __name__ == "__main__":
    success = asyncio.run(test_demo_vm())
