...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('hypervisor_vm_id', sa.String(100), nullable=True, comment='GUID de la VM dans l hyperviseur'),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('pending', 'in_progress', 'creating_vm', 'installing_os', 'post_install', 'installing_software', 'completed', 'failed', 'cancelled', name='deploymentstatus'), nullable=False, server_default='pending'),
